...
const ContraVoucher = (props) => {
    ...
    useEffect(() => {
        ...
        if (contraVouchToEdit) {
            console.log("Context thakle ei IF e dhuke, line 510.....");
            fetch(`${userInfo.Ip}/API/ContraVoucher/Get_VoucherAllInfo?voucherNo=${contraVouchToEdit.VoucherNo}&voucherId=${contraVouchToEdit.VoucherId}`)
                .then(res =>
                    res.json()
                    // console.log(res);
                )
    ...
    useEffect(() => {
        ...
        fetch(`${userInfo.Ip}/API/ContraVoucher/Get_Location?companyId=${userInfo.CompanyId}`)
            .then(res =>
                res.json()
                // console.log(res);
            )
        ...
        fetch(`${userInfo.Ip}/API/ContraVoucher/Get_Project?companyId=${userInfo.CompanyId}`)
            .then(res =>
                res.json()
                // console.log(res);
            )
        ...
        fetch(`${userInfo.Ip}/API/ContraVoucher/Get_AccHeadByCashBank?companyId=${userInfo.CompanyId}&locationId=${userInfo.LocationId}`)
            .then(res =>
                res.json()
                // console.log(res);
            )
        ...
        fetch(`${userInfo.Ip}/API/ContraVoucher/Get_AllBuyerEmpSupLoc?companyId=${userInfo.CompanyId}`)
            .then(res =>
                res.json()
                // console.log(res);
            )
    ...
    const handleAccountHeadSelect = (index, selectedOption) => {
        ...
        const url = `${userInfo.Ip}/API/ContraVoucher/Get_ControlAccount?controlAccId=${selectedOption.ControlAccountId}`;
        fetch(url, {
            method: 'GET',
        })
            .then(res => res.json())
    ...
    const saveVTdata = () => {
        ...
        if (voucherDtTb.length && (total.credit == total.debit) && debitCreditFlag && mandatoryFieldFlag) {
            ...
            axios.post(`${userInfo.Ip}/API/ContraVoucher/Post_CtV_Voucher`, voucherFullData)
                .then(res => {
                    ...
                    if (res) {
                        ...
                        fetch(`${userInfo.Ip}/API/ContraVoucher/Get_VoucherAllInfo?voucherNo=${res.data.VoucherNo}&voucherId=${res.data.VoucherId}`)
                            .then(res =>
                                res.json()
                                // console.log(res);
                            )
                            .then((data) => {
                                ...
                                setContraVouchToEdit(0); //context api voucherEdit state reset
                                assignDataForEdit(data.Voucher.Date, data.Voucher.LocationId, data.Location.Name, data.Voucher.VoucherId, data.Voucher.VoucherNo, data.Voucher.AttachmentId, data.Voucher.ReferenceNo, data.Voucher.Description, data.V_VoucherDetailRows, data.Attachment?.AttachmentData_List);
                            })
                        // reset();
                    }
        ...
        if (voucherDtTb.length && (total.credit == total.debit) && debitCreditFlag && mandatoryFieldFlag && flagSomethingToUpdate) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // setVoucherNumber(res.data);
                    // vouchNoInpRef.current.value = res.data;
                    axios.post(`${userInfo.Ip}/API/ContraVoucher/Update_CtV_Voucher`, voucherFullData)
                        .then(res => {
                            console.log("insert er axios!!")
                            if (res) {
                                ...
                                fetch(`${userInfo.Ip}/API/ContraVoucher/Get_VoucherAllInfo?voucherNo=${res.data.Voucher.VoucherNo}&voucherId=${res.data.Voucher.VoucherId}`)
                                    .then(res =>
                                        res.json()
                                        // console.log(res);
                                    )
    ...
    useEffect(() => {
        return () => {
            ...
            console.log("removing contraVouchToEdit state");
            setJvToEdit(0);
            setContraVouchToEdit(0);
                    ...
                    <div className="block rounded-lg shadow-lg bg-white dark:bg-secondary-dark-bg  text-center">
                        ...
                        <div className=" px-6 pb-4 text-start md:min-h-[60vh] grid grid-cols-4 gap-4 mt-2">
                            ...
                            <fieldset className="md:col-span-4 col-span-4 border border-solid border-gray-300 px-3 pt-1 pb-3 dark:text-slate-50">
                                <legend className="text-sm">Contra Voucher</legend>
